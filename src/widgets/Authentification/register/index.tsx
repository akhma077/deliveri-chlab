  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
      ...
      <div className={styles.error}>
        {errors?.Email && <HelperText isError message="Проверьте данное поле" />}
      </div>
      ...
      <div className={styles.inputPass}>
        ...
        <div className={styles.eyeIcon}>
          {isShowPass['input1'] ? (
            <PassVisibleIcon onClick={() => handleShowPass('input1')} />
          ) : (
            <PassHiddenIcon onClick={() => handleShowPass('input1')} />
          )}
        </div>
      </div>

      <div className={styles.error}>
        {errors?.Password && <HelperText isError message="Проверьте данное поле" />}
      </div>
      ...
      <div className={styles.inputPass}>
        ...
        <div className={styles.eyeIcon}>
          {isShowPass['input2'] ? (
            <PassVisibleIcon onClick={() => handleShowPass('input2')} />
          ) : (
            <PassHiddenIcon onClick={() => handleShowPass('input2')} />
          )}
        </div>
      </div>

      <div className={styles.error}>
        {errors?.PasswordConfirmation && <HelperText isError message="Проверьте данное поле" />}
      </div>
      {status && (
      ...
      <Button variant={ButtonVariant.Base}>
        {isLoading ? 'Загрузка...' : 'Зарегистрироваться'}
      </Button>
      <div className={styles.bott}>
        У вас уже есть аккаунт? <div onClick={() => navigate('/login')}>Войти</div>
      </div>
    </form>
  );
};
